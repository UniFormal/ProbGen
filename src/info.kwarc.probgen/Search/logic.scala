...
case class Path[S, A](start: S, length: Int, _steps: List[(A, S)])
  ...
  override def toString =
    start.toString + steps.map { case (a, s) => s" ~$a~> $s" }.mkString("")
...
/** a search problem, as defined in the lecture except for
...
trait SearchProblem[S, A] {
  ...
  /** the set of goal states (must be a predicate so that searching for them
    * makes sense)
    */
  ...
  /** applies a list of actions in a state, returns the possible resulting
    * states
    */
  ...
  lazy val inapplicableActions: List[(S, A)] = states.flatMap { s =>
    actions.flatMap { a =>
